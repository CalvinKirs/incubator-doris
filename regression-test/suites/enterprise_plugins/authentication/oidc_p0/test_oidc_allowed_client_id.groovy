...
suite("test_oidc_allowed_client_id", "docker,p0") {
    ...
    String suiteName = "test_oidc_allowed_client_id"
    ...
    Map allowedToken = tokensByName["oidc_alice_grafana_query"]
    Map deniedToken = tokensByName["oidc_alice_agent_query"]

    assertEquals("grafana-doris-plugin", allowedToken.clientId)
    assertEquals("agent-gateway", deniedToken.clientId)
    assertTrue(allowedToken.claims.scope.toString().contains("doris.query"))
    assertTrue(deniedToken.claims.scope.toString().contains("doris.query"))
    ...
    oidc.createSingleIntTable(tableName, 22)
    ...
    oidc.createOidcIntegration(integrationName, manifest, [
            ...
            comment: "OIDC allowed client regression integration"
    ])
    oidc.createRoleMapping(
            roleMappingName,
            integrationName,
            roleMappingRules,
            "OIDC allowed client regression mapping")
    sql "sync"
    ...
    assertEquals("OIDC allowed client regression integration", integrationRows[0][3])
    ...
    assertEquals("OIDC allowed client regression mapping", roleMappingRows[0][3])
