...
suite("test_oidc_invalid_token_rejected", "docker,p0") {
    ...
    String suiteName = "test_oidc_invalid_token_rejected"
    ...
    assertEquals(["doris"], (List) aliceToken.claims.aud)
    ...
    assertTrue(integrationRows[0][2].contains("\"oidc.allowed_audiences\" = \"other-service\""))
    assertTrue(integrationRows[0][2].contains("\"oidc.required_scopes\" = \"doris.query\""))
    assertEquals("OIDC invalid token regression integration", integrationRows[0][3])
    ...
    assertEquals("OIDC invalid token regression mapping", roleMappingRows[0][3])
